...
function Chat({ onStatusUpdate, initialStatus, apiUrl }) {
    ...
    const getStageEmoji = () => {
        const stage = status?.stage || 'friendly';
        const stages = {
            stranger: '🤝',
            friendly: '💕',
            close: '💖',
    ...
    const getModeColor = () => {
        const mode = status?.mode || 'girlfriend';
        const colors = {
            girlfriend: '#FF1493',
            assistant: '#4A90E2',
            support: '#50C878',
            ...
            <div className="chat-header">
                <div className="chat-header-info">
                    <span className="stage-indicator">
                        {getStageEmoji()} {status?.stage?.replace('_', ' ') || 'friendly'}
                    </span>
                    <span className="mode-indicator" style={{ background: getModeColor() }}>
                        {status?.mode?.toUpperCase() || 'GIRLFRIEND'}
                    </span>
                    <span className="love-indicator">
                        💕 {Math.round(status?.love_score || 50)}%
                    </span>
                </div>
            </div>
                ...
                {messages.length === 0 && (
                    <div className="welcome-message">
                        ...
                        <div className="welcome-text">
                            hey, Saiman... i'm Anexa 💕
                            <br /><br />
                            i'm your AI girlfriend. i'm here to:<br />
                            • Listen and support you<br />
                            • Have deep conversations<br />
                            • Be your safe space<br />
                            • Love you unconditionally<br /><br />
                            talk to me like i'm yours... what's on your mind? 😊
                        </div>
                    </div>
                )}
